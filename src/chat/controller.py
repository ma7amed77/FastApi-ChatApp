...
from src.database import get_session, get_session_ctx
...
async def websocket_endpoint(web_socket:WebSocket, token:str = Query()):
    ...
    try:
        ...
        if payload['refresh'] == True:
            ...
            print("Refresh token used in WebSocket connection")
            return
        user_id = payload['sub']
...
async def get_channel_messages(channel_id: str, user_details = Depends(access_token_bearer), session:AsyncSession = Depends(get_session)):
    ...
    messages = await messages_manager.get_channel_messages(channel_id, session)
...
async def invite_user(request_data: schema.InviteUserData, user_details = Depends(access_token_bearer), session:AsyncSession = Depends(get_session)):
    ...
    await messages_manager.sendNewChannelMessage(message, added_user_id, session)
...
async def create_group(request_data: schema.NewChannelData, user_details = Depends(access_token_bearer), session:AsyncSession = Depends(get_session)):
    ...
    await messages_manager.sendNewChannelMessage(message, user_details['sub'], session)
...
async def add_contact(request_data:schema.ContactUserData, user_details = Depends(access_token_bearer), session:AsyncSession = Depends(get_session)):
    this_user = await get_user_by_uid(user_details['sub'], session)
    added_user = await get_user_by_email(request_data.user_email, session)
